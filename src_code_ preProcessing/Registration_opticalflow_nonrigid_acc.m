...
fore2_backup = fore2(:,:,1:160);               % wrong order should correct
...
%         [x_batch,y_batch,z_batch] = ind2sub([2^(layer_num - layer) 2^(layer_num - layer) 2^(layer_num - layer)],...
%             find(ones(2^(layer_num - layer), 2^(layer_num - layer), 2^(layer_num - layer))));
%         x_batch = x_batch*batch_size(1) + 0.5;
%         y_batch = y_batch*batch_size(2) + 0.5;
%         z_batch = z_batch*batch_size(3) + 0.5;
...
save(['/work/Mengfan/Embryo/Registration/nonrigid_result_l' num2str(layer_num) '_s'  num2str(smoothness_para)  '_' resize_method '/' num2str(reg_ind)  '.mat'],'phi_current_vec', 'loss', 'mse_ori' ,'-v7.3');
end
