...
tt_ind = tt_ind(2:3);
mkdir([file_path 'deconv\' tt_ind]);
fprintf('\nprocessing %d',tt);
for view = 0:7
    fprintf(' %d ', view);
    data = h5read([file_path source_data],['/t000' tt_ind '/s0' num2str(view) '/0/cells']);
...
%         if iter == 30
%             save([file_path 'deconv\30\' num2str(view)],'data_deconv','-v7.3');
% %         elseif iter == 50
% %             save([file_path 'deconv\50\' num2str(view)],'data_deconv','-v7.3');
%         end
    end
    toc
    save([file_path 'deconv\' tt_ind '\' num2str(view)],'data_deconv','-v7.3');
    tifwrite(uint16(data_deconv),[file_path 'deconv\' tt_ind '\' num2str(view)]);
end
end
